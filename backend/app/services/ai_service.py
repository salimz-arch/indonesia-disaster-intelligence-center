...
from app.models import AIReport, Earthquake, Location, RainfallObservation, WeatherObservation
...
async def _build_context(session: AsyncSession) -> dict:
    ...
    }, quakes, weather, rainfall


async def run_analysis(session: AsyncSession, force: bool = False) -> AIAnalyzeResponse:
    """Analisis lengkap — cache 5 menit (force=true untuk bypass)."""
    if not force:
        cached = await cache_get_json("ai:analysis")
    ...
    output = AISituationOutputSchema.model_validate(
        dict(raw)
    )  # raise bila provider menyimpang dari kontrak
    ...
    # Persist audit trail (kegagalan persist tidak menggagalkan respons)
    try:
        session.add(
            AIReport(
        ...
        logger.exception("gagal persist ai_report — respons tetap dikirim")

    await cache_set_json(
        "ai:analysis", report.model_dump(mode="json"), CACHE_TTL
    )
    return report
